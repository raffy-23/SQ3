            ...
            <form method="post" action="<?= esc(site_url('appearance')) ?>" class="flex flex-col gap-6" style="margin-top: 1rem;">
                ...
                <div style="display:flex; flex-wrap:wrap; gap:.6rem; align-items:center;">
                    <?php foreach (['light' => 'Light mode', 'dark' => 'Dark mode', 'system' => 'Match system'] as $mode => $label): ?>
                        <button type="submit" name="appearance" value="<?= esc($mode) ?>" style="flex:1 1 auto; min-width:7rem;" class="inline-flex items-center justify-content whitespace-nowrap rounded-full text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2 <?= ($appearance ?? 'system') === $mode ? 'bg-primary/10 text-primary border-primary/20' : '' ?>">
                            <?= esc($label) ?>
                        </button>
                    <?php endforeach; ?>
                </div>
